...
const userId = ref<string>(localStorage.getItem('BananaBlogUserId') as string);


onMounted(() => {

    fetchPosts(currentPageNumber.value, selectedSort.value, userId.value);
});

const fetchPosts = (page: number, sort: 'desc' | 'asc', user_id: string) => {


    getPostList(page, sort, '', user_id).then((response) => {
